...
const internModel = require('../models/interModel.js');
const { isValidBody, isValidFullName, isValidEmail, isValidMobile } = require('../util/validator.js');
...
const createInterns = async (req, res) => {
    try {
        ...
        if (!isValidFullName(name)) return res.status(400).send({ status: false, message: `'${name}' this name isn't valid.` });
